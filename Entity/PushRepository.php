 ...
 */
class PushRepository extends CommonRepository
{
    /**
    ...
    public function getEntities($args = [])
    {
        $q = $this->_em
            ...
            ->from('PushBundle:Push', $this->getTableAlias(), $this->getTableAlias().'.id');
    ...
    public function upCount($id, $type = 'sent', $increaseBy = 1)
    {
        try {
            ...
            $q->update(MAUTIC_TABLE_PREFIX.'push_messages')
     ...
     * @return array
     */
    public function getPushList($search = '', $limit = 10, $start = 0, $viewOther = false, $pushType = null)
    {
        ...
        if (!empty($pushType)) {
            $q->andWhere(
                $q->expr()->eq('e.pushType', $q->expr()->literal($pushType))
            );
        }
